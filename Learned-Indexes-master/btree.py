...
class BTree:
    ...
    def _range_search(self, pNode: BTreeNode, k1, k2, res):
        # print(pNode.__str__())
        if pNode is None:
            return
        for i in range(pNode.number):
...
def b_tree_test1(distribution_bulk_load, distribution_insert, is_larger, num):
    if is_larger:
        # 插入的数据比批量加载的数据大，则批量加载数据取前num条，插入数据取后（num*0.08）条
        bulk_load_flag = 0
        insert_flag = 1
    else:
        # 插入的数据比批量加载的数据小，则批量加载数据取后num条，插入数据取前（num*0.08）条
        bulk_load_flag = 1
        insert_flag = 0

    # 批量加载数据
    path_bulk_load = filePath[distribution_bulk_load]
    load_keys, load_values = data_processing(path_bulk_load, bulk_load_flag, num)
    # print(load_keys)

    # 插入操作数据
    path_insert = filePath[distribution_insert]
    insert_keys, insert_values = data_processing(path_insert, insert_flag, int(num * 0.08))
    # print(insert_keys)
    ...
    range_search_result = bt.range_search(4,1000000)
    ...
    delete_bulk_load_search_time = (delete_bulk_load_search_end_time - delete_bulk_load_search_start_time) / len(load_keys)
