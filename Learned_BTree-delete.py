...
def hybrid_training(threshold, use_threshold, stage, stage_nums, core_nums, train_step_nums, batch_size_nums, learning_rate_nums,
    ...
    return index, tmp_inputs[stage_length - 1]

def insert(trained_index, key, stage, stage_set, last_stage):
    for i in range(stage - 1):
        # pick model in next stage
        pre = trained_index[i][pre].predict(key)
        if pre > stage_set[i + 1] - 1:
            pre = stage_set[i + 1] - 1
    if isinstance(trained_index[stage - 1][pre], BTree):  # 最后一层为 B-Tree
        trained_index[stage - 1][pre].insert(key)
    else:  # 最后一层为 Learned Index
        position = int(trained_index[stage - 1][pre].predict(key))
...
def train_index(threshold, use_threshold, stage, distribution, path):
    ...
    # train index
    trained_index, last_stage = hybrid_training(threshold, use_threshold, stage, stage_set, core_set, train_step_set, batch_size_set, learning_rate_set,
    ...
    for ind in range(len(test_set_x)):
        ...
        if isinstance(trained_index[stage - 1][pre], BTree):  # 最后一层为 B-Tree
            ...
            one_error = abs(position - test_set_y[ind])
            if last_stage[pre][position] != total_number[test_set_x[ind]]:
                flag = 1
                off = 1
                # 从预测错误的位置，先向右寻找一位，再向左寻找两位，从而不断寻找左右两边的位置，直到找到正确的位置为止
                second_error = 0
                while last_stage[pre][position] != test_set_x[ind]:
                    position += flag * off
                    flag = -flag
                    off += 1
    end_time = time.time()
